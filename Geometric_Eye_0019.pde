...
void setup()
{
  ...
  // noLoop(); // just draw and save once
}
...
void draw() {
  ...
  if (keyPressed) {
    if (mousePressed) {
      ...
      myFloodFill.DoFill(mouseX, mouseY, color(0, 0, 0));  // fill colour
      updatePixels();

      //  save("test_" + saveCount + ".png");
    }
  }

  if (outlinesDrawn == false) {
    drawOutlines();
    //  save("test.png");
  }
}
...
void drawOutlines() {
  ...
  fillPoints(scale * 0.098, 33, 0, 0, 0, 0);
  fillPoints(scale * 0.15, 36, 0, 0, 0, 0);
